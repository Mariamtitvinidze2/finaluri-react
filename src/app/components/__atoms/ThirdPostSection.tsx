    ...
    <div className={`max-w-[550px] mx-auto ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
      ...
      <div className={`rounded-lg shadow-md p-3 mb-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
        <div className={`flex gap-3 items-center border-b pb-3 ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          />
          <input
            className={`p-2 rounded-full w-full focus:outline-none cursor-pointer transition pl-4 ${theme === 'dark'
        ...
        <div className='flex justify-between pt-2 px-2'>
          <button className={`flex items-center gap-1 p-2 rounded-lg text-sm ${theme === 'dark' ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'
            }`}>
            <FaVideo className='text-red-500 text-lg' /> Live video
          </button>
          <button
            onClick={() => setIsModalOpen(true)}
            className={`flex items-center gap-1 p-2 rounded-lg text-sm ${theme === 'dark' ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'
              }`}
          >
      ...
      {isModalOpen && (
        <div className='fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50'>
          <div className={`w-[500px] rounded-lg shadow-lg overflow-hidden ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'
            }`}>
            <div className={`flex justify-between items-center p-4 border-b ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'
              ...
              <IoClose
                className={`text-2xl cursor-pointer p-1 rounded-full transition ${theme === 'dark' ? 'text-gray-400 hover:bg-gray-700' : 'text-gray-500 hover:bg-gray-100'
                  }`}
                onClick={() => {
                  setIsModalOpen(false);
                  setImage(null);
            ...
            <div className='flex gap-3 items-center p-4'>
              ...
              <div>
                ...
                <select
                  className={`text-xs border-none rounded p-1 cursor-pointer hover:bg-opacity-80 transition ${theme === 'dark'
            ...
            <textarea
              className={`w-full min-h-[100px] p-4 text-lg border-none focus:outline-none resize-none ${theme === 'dark'
            ...
            ></textarea>
            {image && (
              <div className={`relative mx-4 mb-4 rounded-lg overflow-hidden ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'
                } border`}>
                <Image
                  src={image}
                  alt='Selected'
                ...
                <button
                  ...
                  className={`absolute top-2 right-2 text-white rounded-full p-1 hover:bg-opacity-70 transition ${theme === 'dark' ? 'bg-gray-700 bg-opacity-50' : 'bg-gray-800 bg-opacity-50'
                    }`}
                >
                  <IoClose size={20} />
                </button>
            ...
            <div className={`flex justify-between items-center p-3 mx-3 mb-3 rounded-lg ${theme === 'dark'
                ? 'bg-gray-700 border-gray-600'
                : 'bg-gray-50 border-gray-200'
              } border`}>
              <span className={`font-medium ml-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'
                }`}>Add to your post</span>
              <div className='flex gap-1'>
                <button
                  onClick={triggerFileInput}
                  className={`hover:bg-opacity-20 p-2 rounded-full transition ${theme === 'dark' ? 'text-green-400 hover:bg-gray-600' : 'text-green-600 hover:bg-gray-200'
                    }`}
                  title="Photo"
                >
                  <FaImage size={20} />
      ...
      <div className='space-y-4'>
        {posts.map((post) => (
          <div key={post.id} className={`p-4 rounded-lg shadow-md ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'
            }`}>
            ...
            <div className='flex items-center justify-between mb-3'>
              ...
              <div className='relative group'>
                <button className={`p-2 rounded-full transition ${theme === 'dark' ? 'text-gray-400 hover:bg-gray-700' : 'text-gray-500 hover:bg-gray-100'
                  }`}>
                  <FaEllipsisH />
                </button>
                <div className={`absolute right-0 top-8 shadow-lg rounded-md py-1 w-40 z-10 hidden group-hover:block ${theme === 'dark' ? 'bg-gray-700' : 'bg-white'
                  }`}>
                  <button
                    onClick={() => handleEditPost(post)}
                    className={`flex items-center gap-2 w-full px-4 py-2 text-left ${theme === 'dark' ? 'hover:bg-gray-600' : 'hover:bg-gray-100'
                      }`}
                  >
            ...
            {post.image && (
              ...
              >
                <Image
                  ...
                  src={post.image}
                  alt='Post'
                  className={`w-full rounded-lg ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'
                    } border`}
                />
            ...
            <div className={`flex items-center justify-between border-t pt-2 ${theme === 'dark' ? 'border-gray-700 text-gray-400' : 'border-gray-200 text-gray-500'
              }`}>
              <button
                onClick={() => post.id && handleLike(post.id)}
                className={`flex items-center gap-1 px-2 py-1 rounded-md transition ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-100'
            ...
            {post.id && showComments[post.id] && (
              <div className={`mt-3 pt-3 ${theme === 'dark' ? 'border-t border-gray-700' : 'border-t border-gray-200'
                }`}>
                  ...
                  {comments[post.id]?.map((comment) => (
                    <div key={comment.id} className='flex gap-2 group'>
                      <Image
                        ...
                        className='rounded-full object-cover border border-gray-200'
                      />
                      <div className={`rounded-lg p-2 flex-1 relative ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-100'
                        }`}>
                        ...
                        <div className='flex items-center gap-2 mt-1'>
                          <button
                            ...
                            className={`text-xs ${comment.likes?.includes(currentUserId)
                        ...
                        <button
                          ...
                          className={`absolute -right-2 -top-2 p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity ${theme === 'dark' ? 'bg-gray-600 text-gray-300' : 'bg-gray-200 text-gray-600'
                            }`}
                ...
                <div className='flex gap-2 items-center'>
                  ...
                  <div className='flex-1 flex gap-2'>
                    <input
                      ...
                      className={`flex-1 rounded-full px-3 py-1 text-sm focus:outline-none ${theme === 'dark'
